...
const Header = () => {
  ...
  return (
    <header className="fixed z-50 p-3 w-screen px-4 md:p-6 md:px-16 bg-primary">
          {/* desktop and Tablet */}
        <div className="hidden md:flex w-full h-full items-center justify-between">
            ...
            <div className="flex items-center gap-8">
                ...
                <div className="relative">
                  <motion.img whileTap={{scale:0.6}} src={user ? user.photoURL : Avatar} className="rounded-full w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer" alt="userProfile" onClick={login} />
                </div>

                {
                  isMenu && (
                    ...
                    className="w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute right-16 top-20">

                  {
                      user && user.email === "rajatsahu1301@gmail.com" && (
                        <Link to={"/createitem"}>
                          <p className="px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-out text-textColor text-base" onClick={()=>setIsMenu(false)}>New Item <MdAdd/></p>
                        </Link>
                      )
                    }

                    <p className="px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-out text-textColor text-base" onClick={Logout}>Logout <MdLogout/></p>
          ...
          <MdShoppingBasket className="text-textColor text-2xl cursor-pointer"/>
                ...
                </div>
            {
                ...
                <ul
                    ...
                    <li className="hover:bg-slate-100 px-4 py-2 text-base text-textColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">Service</li>
                </ul>

                    <p className="m-2 p-2 rounded-md shadow-md flex justify-center
                    bg-gray-200 items-center gap-3 cursor-pointer hover:bg-gray-300 transition-all duration-100 ease-out text-textColor text-base" onClick={Logout}>Logout <MdLogout/></p>
